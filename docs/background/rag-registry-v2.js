...
// ─── REGISTRY ENTRIES ────────────────────────────────────────────────────────
//
// RULES FOR THIS ARRAY (enforced by Registry Governance, Section 8)
//
// 1. Every file that enters the SprintZero system is registered here FIRST.
//    No unregistered files in production.
//
// 2. desc fields contain only: what the file IS and what rules/protocol it holds.
//    Examples of application belong in the file itself, not in this registry entry.
//    The registry is an index — not a summary of contents.
//
// 3. topics are loose retrieval tags. They are not an ontology.
//    Approximate is acceptable. Missing a tag is not a failure — wrong tags are.
//
// 4. strength ratings:
//    canonical   — foundational literature, universally applicable principles
//    established — validated practice across multiple projects
//    anecdotal   — single-project data, not yet cross-validated
//
// 5. Status lifecycle: PLANNED → IN PROGRESS → EXISTS → ARCHIVED
//    TEMPLATE = permanent slot placeholder, never promoted.
//    Update status here before updating the file, not after.
//
// 6. Situational files are project-scoped. On project close: status → ARCHIVED,
//    add closing_date field. Entry is never deleted.
//
// ─────────────────────────────────────────────────────────────────────────────
...
const entries = [
  ...
  // ── FOUNDATIONAL ────────────────────────────────────────────────────────
  // WHY layer. Universal principles. No project-specific content.
  // One concept per file, fully explained.

  { id:"F-001", col:"Foundational", file:"ux_principles_and_heuristics.md",
    desc:"Core UX heuristics (Nielsen, Gestalt), cognitive load, mental models, affordances. Principles that apply regardless of product domain or research method.",
    topics:"usability, heuristics, cognitive load, mental models, affordances",
    domain:"general", strength:"canonical", status:"PLANNED" },

  { id:"F-002", col:"Foundational", file:"cognitive_biases_in_ux_research.md",
    desc:"Taxonomy of cognitive biases that distort UX research: confirmation bias, availability heuristic, framing effects, social desirability. Covers both data collection and synthesis stages.",
    topics:"bias, research quality, synthesis errors, framing, confirmation bias",
    domain:"general", strength:"canonical", status:"PLANNED" },

  { id:"F-003", col:"Foundational", file:"research_methods_reference.md",
    desc:"Method selection rules: when each research method is appropriate, its signal-to-noise characteristics, and its generalisability limits. Covers interviews, surveys, diary studies, App Store reviews, NPS, session recordings.",
    topics:"interviews, surveys, diary studies, app store, NPS, generalisability, methods",
    domain:"general", strength:"canonical", status:"PLANNED" },

  { id:"F-004", col:"Foundational", file:"ai_capabilities_and_constraints.md",
    desc:"Reliable and unreliable LLM behaviours in research reasoning contexts. Covers confabulation mechanics, hallucination patterns, and the RAG vs pure generation distinction.",
    topics:"llm, hallucination, confabulation, rag, reliability, constraints",
    domain:"general", strength:"canonical", status:"PLANNED" },

  { id:"F-005", col:"Foundational", file:"insight_synthesis_failure_modes.md",
    desc:"Eight failure modes in AI-assisted UX research synthesis: misclassification, overgeneralisation, theme conflation, transcript literalism, confabulation at scale, consistency collapse, insight ephemerality, trust without process. Grounded in NatWest practitioner evidence (Ogilvy) and SprintZero testing.",
    topics:"misclassification, synthesis, theme conflation, insight quality, failure modes, pitfalls",
    domain:"general", strength:"canonical", status:"EXISTS" },

  // ── APPLIED ─────────────────────────────────────────────────────────────
  // HOW layer. Protocols and procedures. Reusable across domains and projects.
  // One solution with rationale per file.

  { id:"A-001", col:"Applied", file:"sprintzero_evaluation_framework.md",
    desc:"The SprintZero evaluation protocol. Specifies: STRONG / UNCERTAIN / WEAK verdict thresholds, classification logic (Corroboration / Contradiction / Expansion / Net New), falsification test requirements, transfer assumption check procedure, and the decision-change vs decision-enable distinction. Reusable across all corpora.",
    topics:"evaluation, verdicts, falsification, transfer assumption, protocol, classification",
    domain:"general", strength:"canonical", status:"EXISTS" },

  { id:"A-002", col:"Applied", file:"corpus_annotation_protocol.md",
    desc:"Rules for annotating a SprintZero corpus: evidence unit ID schema (R01-Rn, INT-01-INTn), required fields per unit (source type, date, participant ID, behavioural signal tags, sentiment, intent), counter-signal identification rules, and confidence scoring inputs.",
    topics:"annotation, corpus, review ids, signals, counter-signals, confidence, tagging",
    domain:"general", strength:"established", status:"PLANNED" },

  { id:"A-003", col:"Applied", file:"rag_registry_management.md",
    desc:"Governance rules for the RAG Registry: how to add, update, retire, and gap-flag corpora. Versioning conventions, metadata schema, status lifecycle, and gap flag output format.",
    topics:"registry, metadata, versioning, gap flags, corpus management, governance",
    domain:"general", strength:"established", status:"IN PROGRESS" },

  { id:"A-004", col:"Applied", file:"sprintzero_copilot_role_task.txt",
    desc:"Role and task definition for the SprintZero Research Copilot. Specifies the eight-step evaluation procedure, constraints on AI behaviour, and the refusal protocol for structurally absent evidence. Does not contain output format — see A-006.",
    topics:"copilot, role, task, evaluation procedure, refusal, constraints, system prompt",
    domain:"general", strength:"established", status:"EXISTS" },

  { id:"A-005", col:"Applied", file:"benchmark_query_set.md",
    desc:"15 benchmark queries for evaluating RAG retrieval quality across build phases. Specifies expected retrieval characteristics, source diversity requirements, and failure mode triggers for each query.",
    topics:"evaluation, benchmarks, testing, retrieval quality, failure modes, qa",
    domain:"general", strength:"established", status:"PLANNED" },

  { id:"A-006", col:"Applied", file:"sprintzero_output_schema.xml",
    desc:"XML schema defining the required structure for every SprintZero response. Specifies all fields, required/conditional status, trigger conditions, and field-level rules. The application rendering layer reads this schema. The AI produces to this schema. No display logic here.",
    topics:"output, schema, xml, response format, structure, rendering contract",
    domain:"general", strength:"canonical", status:"EXISTS" },

  { id:"A-007", col:"Applied", file:"qa_agent_protocol.md",
    desc:"Protocol for the QA Validation Agent (Agent 2). Specifies the adversarial review procedure applied to Agent 1 output: reasoning validity check, counter-signal audit, confidence score challenge, transfer assumption depth check, and behavioural mechanism specificity test. Output schema: qa_review element appended to SprintZero_response.",
    topics:"qa, agent, validation, adversarial review, reasoning check, confidence audit",
    domain:"general", strength:"established", status:"PLANNED" },

  // ── CONTEXTUAL ──────────────────────────────────────────────────────────
  // WHERE layer. Domain-specific constraints and knowledge.
  // One domain scenario per file. Required for transfer assumption reasoning.

  { id:"C-001", col:"Contextual", file:"domain_weather_apps.md",
    desc:"Weather app domain knowledge required for transfer assumption reasoning. Covers: user trust formation patterns, data freshness expectations, Personal Weather Station density by geography, national meteorological service alternatives (MeteoSwiss, DWD, Météo-France), and EU vs US weather severity profiles.",
    topics:"weather, trust, data freshness, PWS density, EU transfer, meteorological services, national met",
    domain:"weather", strength:"established", status:"PLANNED" },

  { id:"C-002", col:"Contextual", file:"domain_fintech_compliance.md",
    desc:"Fintech UX domain constraints. Covers: regulatory onboarding requirements, KYC/AML friction patterns, user trust formation in financial data contexts, EU vs US regulatory structural differences.",
    topics:"fintech, compliance, onboarding, KYC, AML, regulatory, trust",
    domain:"fintech", strength:"established", status:"PLANNED" },

  { id:"C-003", col:"Contextual", file:"domain_b2b_saas.md",
    desc:"B2B SaaS UX domain constraints. Covers: multi-stakeholder decision-making dynamics, champion vs end-user vs buyer role distinctions, enterprise onboarding complexity, admin/user permission patterns.",
    topics:"b2b, saas, enterprise, stakeholders, onboarding, admin, permissions",
    domain:"b2b-saas", strength:"established", status:"PLANNED" },

  // ── SITUATIONAL ─────────────────────────────────────────────────────────
  // THIS layer. Project-specific evidence corpora.
  // Chunked by topic, speaker, or phase. Citable by unit ID.
  // Retired (not deleted) when project closes.

  { id:"S-001", col:"Situational", file:"corpus_weather_underground_app_store_reviews.md",
    desc:"13 annotated App Store reviews for Weather Underground (R01-R13). Each unit carries: date, star rating, behavioural signal tags, sentiment, user intent, and counter-signal flags. The founding corpus. S-001 is the evidence base for all Phase 1 benchmark queries.",
    topics:"weather underground, app store, reviews, trust, stale data, forecast accuracy, cache",
    domain:"weather", strength:"canonical", status:"EXISTS" },

  { id:"S-002", col:"Situational", file:"[project]_[source]_[date].md",
    desc:"Template slot for future project-specific corpora. Replace with actual project name, source type, and date range. Each project generates one or more Situational files: interview transcripts (chunked by participant or session), survey verbatims, NPS comments, support tickets.",
    topics:"project-specific, transcripts, surveys, NPS, support tickets",
    domain:"project", strength:"anecdotal", status:"TEMPLATE" },
...
function makeBuildTable() {
  ...
  const rows = [
    ["Phase 1","F-005, A-001, S-001","Raw embedding only. No metadata. No scoring boosts. Run 5 benchmark queries. Establish baseline. Add nothing until a failure justifies it."],
    ...
    ["Phase 3","A-004, A-006, A-007","Separation of concerns refactor. Role/task (A-004) and output schema (A-006) split from prompt template. QA agent protocol (A-007) registered. Composition prompt updated to reference schema. Full 15-query benchmark run (A-005)."],
    ["Phase 4","F-001, F-004, A-003, A-005, S-002+","Add diversity enforcement. Add Situational project corpora. Add gap flagging. Foundational injection live. Registry management file complete. Benchmark set published."],
...
const doc = new Document({
  ...
  sections: [{
    ...
    headers: {
      default: new Header({ children: [
        new Paragraph({
          ...
          children:[
            new TextRun({text:"SprintZero  ·  RAG Registry V2  ·  Phase 3",font:"Arial",size:18,color:BLUE_MID}),
    ...
    children: [
      ...
      new Paragraph({...sp(0,60), children:[new TextRun({text:"Version 2  ·  Phase 3  ·  May 2026",font:"Arial",size:24,color:GREY_TEXT})]}),
      rule(BLUE_DARK),
      gap(),
      new Paragraph({...sp(0,100), children:[new TextRun({text:"The index of all knowledge available to the SprintZero reasoning system. Specifies what exists, what is planned, what is missing, and how each file is retrieved. This is a rules-only document. Examples live in the files they index.",font:"Arial",size:22,color:"333333",italics:true})]}),
      new Paragraph({...sp(0,0), children:[new TextRun({text:"Phase 3 refactor: Registry contains rules only. Output schema, copilot role/task, and QA agent protocol are now registered as independent Applied files.",font:"Arial",size:22,color:GREY_TEXT})]}),
      gap(),
      ...
      gap(),
      body("Phase 3 refactor principle: the Registry contains rules only. It is an index, not a summary. Examples of application belong in the files they index. This means: descriptions in the registry state what a file IS and what rules or protocols it holds — not what it contains or how it should be used. That separation makes each component independently maintainable and independently replaceable.", {italic:true}),
      gap(),
      ...
      gap(),
      body("Every file that enters the SprintZero system must be registered here before use. Status is updated as files are created. Descriptions state what a file IS — not what it contains. Examples belong in the files themselves."),
      gap(),
      // Legend
      (() => {
      ...
      gap(),
      gap(),
      callout("Phase 3 is now. Separation of concerns: role/task, output schema, and QA agent protocol are registered as independent files. The rendering layer reads the schema. The AI produces to the schema. They never need to know about each other.", BLUE_MID),
      gap(),
      ...
      rule(BLUE_DARK),
      gap(),
      new Paragraph({...sp(0,0), alignment:AlignmentType.CENTER, children:[new TextRun({text:"SprintZero  ·  RAG Registry V2  ·  Phase 3  ·  Niko  ·  May 2026",font:"Arial",size:18,color:GREY_TEXT,italics:true})]}),
    ]
  }]
});

Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync("/mnt/user-data/outputs/SprintZero_RAG_Registry_V2.docx", buf);
  console.log("Done.");
});
